...
export const useConfigStore = defineStore('config', () => {
  ...
  const actionSchemes = computed(() => config.value!.actionSchemes!)
  ...
  return {
    ...
    changeHotkey, removeHotkey, addHotKey, translate, actionSchemes,
...
function fetchConfig() {
  ...
  watch(data, (val) => {
    ...
    // 初始化选中动作方案 (后加的字段, 旧的 config.json 没有)
    if (!val.actionSchemes) {
      val.actionSchemes = []
    }
    config.value = val
  })
