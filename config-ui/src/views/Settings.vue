...
const { customKeymaps, customParentKeymaps, customSonKeymaps, options, keymaps, config } = storeToRefs(useConfigStore())
...
const resetOtherToFalse = (newValue: boolean) => {
  [showMouseOption, showLanguageOption, showKeyboardLayout, showKeymapDelay, showSkin, showHelpPage].forEach(x => x.value = false)
  return newValue
}
      ...
      <v-col class="ml-5 mr-3">
        <div class="otherSetting">
          <v-row :dense="true">
            <v-col>
              <v-card :title="translate('label:505')" min-width="180">
                <v-card-text>
                  ...
                  <v-btn class="mt-3 mr-2 text-none" width="170" color="blue" variant="outlined" @click="showHelpPage = resetOtherToFalse(!showHelpPage)">帮助页面</v-btn>
                </v-card-text>
              </v-card>
            </v-col>
          </v-row>
            ...
            <v-row v-show="showHelpPage">
              <v-col>
                <v-card title="帮助页面 (HTML)" elevation="2">
                  <v-card-text>
                    <p class="text-body-2 mb-2">填写自定义 HTML 帮助页, 保存后生成 bin/site/help.html, 可通过 MyKeymap 动作 openHelpHtml 打开; 清空后删除生成的文件</p>
                    <v-textarea color="primary" variant="underlined" auto-grow rows="6" v-model="config.helpPageHtml" placeholder="<h1>我的使用说明</h1>"></v-textarea>
                  </v-card-text>
                </v-card>
              </v-col>
            </v-row>
